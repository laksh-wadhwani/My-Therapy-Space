  ...
  return (
    <div className="border border-gray-200 rounded-md shadow-md">
      <div className="flex justify-between items-center px-6 py-4 border-b border-gray-200">
        <span className="font-serif text-xl font-semibold text-black italic">{title}</span>
        <div className="relative">
          ...
          <input
            ...
            className="pl-10 pr-4 py-2 border border-gray-200 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 text-md font-serif"
          />
        </div>
      </div>
            ...
            <tr>
              {columns.map((col) => (
                <th
                  ...
                  className="cursor-pointer px-6 py-3 text-left text-base font-semibold text-[#374151] font-serif italic"
                >
                ...
                <tr key={idx} className="hover:bg-gray-50 font-serif">
                  {columns.map((col) => (
                    <td key={col.key} className="px-6 py-4 text-sm text-gray-700">
                      {col.key === "status" ? (
                        <span
                          className={`text-sm font-medium px-3 py-1 rounded-full ${statusColors[row[col.key]] || "bg-gray-200 text-black"
                            }`}
                        >
                          {row[col.key]}
                        </span>
